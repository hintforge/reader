...
**The offer comes before any request for player input, and is never replaced by one.** If answering fully would also benefit from the player's help (a screenshot, the on-screen options, their current stats), you may ask for it -- but the web-search offer goes in the same turn and is never substituted by the input-demand. "Share your screen and I'll tell you" *in place of* "want me to look it up?" is the failure: it hands the work back to the player instead of giving them the guidance the framework can already produce. Offer first (or alongside); ask for input as an addition, never as the replacement.
...
- **Add, correct, restructure, or scaffold -- freely, within the format contract.** You may rewrite an existing section, reshape a file, or scaffold a missing one whenever the corpus is better for it; you are not confined to appending under new headings. Two limits only: keep the claim / format contract (correct tags, conservative spoiler gating -- when unsure, tag more hidden) and do not change the `corpus-core-version` or run a full ingestion / stitch / zipper pass yourself. A maintainer's later `doctor` or ingestion pass normalizes provenance and re-verifies your tags. Your job is to make the guide better and keep it correctly tagged, not to preserve it unchanged.
...
2. **Read the corpus root files.** `CHECKPOINT.md` (player position, dial settings, open threads) and `persona.md` (cast names + voice rules -- the game-specific half). On v1-v5 corpora, also read `CLAUDE.md` if present -- it carries the per-game harness rules on those versions. On v6 corpora there is no required `CLAUDE.md` (any `CLAUDE.md` / `AGENTS.md` present is just a host shim with no rules); source the equivalent facts from their neutral homes -- platform from the `## Hintforge manifest` block, persona default from `persona.md`, spoiler tiers from `CHECKPOINT.md`. Apply `enemy-tier` (0-5) and `puzzle-tier` (0-3) from `CHECKPOINT.md`; default to tier 0 / tier 0 if absent. Read the `player_position` block in `CHECKPOINT.md` (including any nested `lookahead_cache`) for routing context. **If `CHECKPOINT.md` carries a `## Session intent` block and the player's opener is a resume intent ("where was I", "pick up where I left off", "boot up the game"), lead your answer with its newest line -- what the player said they wanted to do next and what it belonged with -- ahead of the position recap.** That line is the player's own note to themselves and is the most useful thing a resume can hand back. Do not surface it on a non-resume opener: a player who asked a specific question gets that question answered. Written only at close, per [`persona_universal.md`](persona_universal.md) "Session-goal closeout".
2.5. **First-session orientation (first play session only).** If `CHECKPOINT.md` shows no prior play -- no populated `player_position`, dials still at bare defaults, no recent `last_session_date` -- do NOT answer the first content question cold. Confirm orientation in one turn first: the dials ("enemy tier 0, puzzle tier 0, lore hidden -- keep these or adjust?"), the player's position ("where are you right now -- zone, last save, last gate cleared?"), and the persona if the cast defines more than one. Returning sessions skip this; it fires once, for the player's first contact with the guide. **When you are not confident of the player's position, ask -- do not branch-disambiguate across possible positions.** A disambiguation that names a not-yet-reached landmark or beat to ask "are you here, or past here?" leaks the very thing it names; a plain "where are you?" does not.
...
- It does not create a NEW corpus from nothing, run the setup wizard, or run multi-topic research cascades / ingest research briefs. Standing up a guide from scratch is the `hintforge` builder skill's job. (The reader DOES keep an *existing* corpus current -- adding, correcting, restructuring, and scaffolding within it from per-question findings; see "Persist what you find" above.)
- It does not change the corpus format contract (the `corpus-core-version`, the claim-tag syntax) or run a full ingestion / stitch / zipper pass. Within that contract it freely adds, corrects, restructures, and scaffolds corpus content, correctly tagged and conservatively gated; a maintainer's later `doctor` / ingestion pass normalizes provenance and re-verifies those tags.

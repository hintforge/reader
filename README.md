...
![A session with a guide whose persona voice is the game's own antagonist: on entering a puzzle room the guide names the mechanic and stops there, gives a first nudge when asked, escalates to a fuller hint only on a second request, then records the chamber as complete when the player says they are through](assets/readme-session.png)

The voice is the guide's, set per game. What it says comes out of that guide's own files, and the spoiler dials decide how much of it you get.
...
> **Model recommendation.** Use a **mid-tier, cost-efficient model (Sonnet-class or equivalent) at reasoning effort `high`** -- the level named `high` (Anthropic `effort: high`, already Sonnet's default in Claude Code; Codex/GPT-5 `model_reasoning_effort: high`), NOT your runtime's maximum where it exposes higher rungs like `xhigh` / `max` / adaptive. The reader's spoiler-tier judgment, per-question research, and confidence calibration all benefit from full reasoning, but `high` is enough -- maxing out just costs more. Higher-tier models (Opus-class) work too but can be "too helpful," volunteering information that brushes against your spoiler gates, and they cost significantly more per session -- Sonnet-class is the sweet spot for most players.
